...
class SRSender:
    def __init__(self, senderSocket, address, timeout=TIMEOUT,
        ...
        self.acks = [False] * 256  # 标记数据分组是否已被确认

    def udp_send(self, pkt):  # 使用 udp 发送数据分组
    ...
    def wait_ack(self):
        ...
        while True:
            ...
            try:
                ...
                if ack_seq in range(self.send_base, self.send_base + self.window_size):  # 受到确认分组在窗口范围内
                    self.acks[ack_seq] = True

                if ack_seq == self.send_base:  # 如果是窗口中最小的
                    # 滑动窗口
...
class SRReceiver:
    ...
    def __init__(self, receiverSocket, timeout=10, windowSize=WINDOW_SIZE, lossRate=0):
        ...
        self.window_size = windowSize
        self.loss_rate = lossRate
        self.recv_base = 0  # 接收窗口头部序列号
        self.recvs = [None] * 256  # 收到的数据分组
        self.target = None  # 确认分组的发送目标地址（即发送端地址）

    def udp_send(self, pkt):  # 确认分组的发送目标地址（即发送端地址）
